/**
 * Test: Generate ONE background prompt in ALL aspect ratios
 * Direct Gemini API integration (bypasses Next.js auth)
 */
...
async function uploadToGoogleDrive(buffer, fileName, folderId, mimeType) {
  ...
  // Get public URL using Google Drive thumbnail API
...
async function testOneBackgroundAllFormats() {
  console.log('🧪 Test: Generate ONE background in ALL aspect ratios\n')
  ...
  try {
    ...
    // Generate background in each format
    for (const formatConfig of FORMATS) {
      try {
        ...
        const slug = await generateSlug(`vibrant-pastel-${formatConfig.format}`)
        const fileName = `${slug}_${Date.now()}.jpg`

        console.log(`   📤 Uploading to Google Drive: backgrounds/${formatConfig.folder}/${fileName}`)
        ...
        const dbResult = await sql`
          INSERT INTO backgrounds (
            ...
            ${slug + '-' + Date.now()},
            ${'Test background for ' + formatConfig.description},
          ...
          RETURNING id, name, format, width, height, storage_path
        `

        console.log(`   ✅ Saved to database`)
        console.log(`      ID: ${dbResult[0].id}`)
        console.log(`      Path: ${dbResult[0].storage_path}`)
    ...
    console.log('\n🎉 Test complete!')
    console.log('📝 Next: Check Google Drive to see backgrounds in each format folder')
...
testOneBackgroundAllFormats().then(exitCode => {
