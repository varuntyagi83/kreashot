...
export function FinalAssetsWorkspace({ categoryId, format = '1:1' }: FinalAssetsWorkspaceProps) {
  ...
  // PIL-rendered preview state — always the source of truth for the preview panel
  const [pilPreviewData, setPilPreviewData] = useState<string | null>(null)
  const [pilPreviewStale, setPilPreviewStale] = useState(false)
  ...
  // Mark PIL preview as stale whenever any setting that affects the output changes.
  // The stale flag shows an "outdated" badge over the last rendered image rather than blanking it.
  ...
  useEffect(() => {
    const key = [
      ...
      selectedTemplateId,
      format,
      JSON.stringify(freeformTexts.map(t => ({ x: t.x, y: t.y, w: t.width, fs: t.fontSize, c: t.color, text: t.text, font: t.fontUrl }))),
      logoPosition, logoSize, logoXPx, logoYPx,
    ].join('|')
    if (prevPreviewKeyRef.current && key !== prevPreviewKeyRef.current) {
      setPilPreviewStale(true)
    }
    prevPreviewKeyRef.current = key
  }, [previewImageUrl, selectedLogoId, selectedCopyDocId, selectedTemplateId, format, freeformTexts, logoPosition, logoSize, logoXPx, logoYPx])

  // Auto-generate PIL preview on first load once a base image is available
              ...
              <div className="space-y-2">
                ...
                {previewImageUrl ? (
                  <div className="border border rounded-lg p-3 space-y-2">
                    ...
                    >
                      {pilPreviewData ? (
                        /* PIL-rendered image — always shown once available */
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={pilPreviewData} alt="Preview" className="w-full h-full object-cover" />
                      ) : (
                        /* First load: base image dimmed while PIL renders */
                        <Image src={previewImageUrl} alt="Base image" fill className="object-cover object-center opacity-40" />
                      )}
                      ...
                      {/* Stale badge: settings changed since last render */}
                      {pilPreviewStale && !isGeneratingPreview && (
                        <div className="absolute top-2 left-2 bg-amber-500/90 backdrop-blur-sm text-white text-xs px-2 py-1 rounded flex items-center gap-1">
                          <span>Settings changed — click Refresh Preview</span>
                        </div>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground">
