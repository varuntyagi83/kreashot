const http = require('http');
const https = require('https');
const { createClient } = require('@supabase/supabase-js');
const { google } = require('googleapis');
const fs = require('fs');
const path = require('path');

// Load env
const envContent = fs.readFileSync(path.join(__dirname, '.env.local'), 'utf8');
const env = {};
envContent.split('\n').forEach(line => {
  const match = line.match(/^([^=]+)=(.*)$/);
  if (match) {
    env[match[1]] = match[2].replace(/^["']|["']$/g, '');
  }
});

// Decode the base64 auth token
const authCookie = '';

const authData = JSON.parse(Buffer.from(authCookie, 'base64').toString());
const accessToken = authData.access_token;
const userId = authData.user.id;

console.log('✓ Auth token decoded');
console.log(`  User: ${authData.user.email}`);
console.log(`  User ID: ${userId}`);

const CATEGORY_ID = '530baae1-19c7-44d4-b6da-4efa1d0b8d47';
const BASE_URL = 'http://localhost:3006';

// Make authenticated request using cookies
async function callAPI(endpoint, method, body) {
  return new Promise((resolve, reject) => {
    const url = new URL(`${BASE_URL}${endpoint}`);
    const cookies = `sb-raedrrohryxpibmmhcjo-auth-token=base64-${authCookie}`;
    
    const options = {
      hostname: url.hostname,
      port: url.port || 3006,
      path: url.pathname + url.search,
      method,
      headers: {
        'Content-Type': 'application/json',
        'Cookie': cookies,
      },
    };

    const req = http.request(options, (res) => {
      let data = '';
      res.on('data', (chunk) => { data += chunk; });
      res.on('end', () => {
        try {
          resolve({ status: res.statusCode, data: JSON.parse(data) });
        } catch (e) {
          resolve({ status: res.statusCode, raw: data });
        }
      });
    });

    req.on('error', reject);
    if (body) req.write(JSON.stringify(body));
    req.end();
  });
}

async function main() {
  const supabase = createClient(env.NEXT_PUBLIC_SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
  const drive = google.drive({
    version: 'v3',
    auth: new google.auth.GoogleAuth({
      credentials: {
        client_email: env.GOOGLE_DRIVE_CLIENT_EMAIL,
        private_key: env.GOOGLE_DRIVE_PRIVATE_KEY?.replace(/\\n/g, '\n'),
      },
      scopes: ['https://www.googleapis.com/auth/drive'],
    }),
  });

  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('IMAGE PIPELINE AUDIT - STEP 2: Generate + Save Background');
  console.log('Date: March 18, 2026, 2:25 PM');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

  // Step 2a: Generate
  console.log('\n=== Step 2a: Generate Background ===');
  console.log('Calling: POST /api/categories/{id}/backgrounds/generate');
  
  const genResp = await callAPI(`/api/categories/${CATEGORY_ID}/backgrounds/generate`, 'POST', {
    prompt: 'Professional product photography scene - PIPELINE AUDIT March 18 2026 at 2:25 PM',
    format: '1:1',
    count: 1,
  });

  if (genResp.status !== 200) {
    console.error('✗ Generation failed:', genResp.status, genResp.raw || genResp.data);
    process.exit(1);
  }

  console.log('✓ Background generated');
  const bg = genResp.data.backgrounds[0];
  console.log(`  Format: ${bg.format} (${bg.width}x${bg.height})`);
  console.log(`  MIME: ${bg.mimeType}`);
  console.log(`  Image data length: ${bg.imageData.length} chars`);

  // Step 2b: Save
  console.log('\n=== Step 2b: Save Background ===');
  console.log('Calling: POST /api/categories/{id}/backgrounds');
  
  const timestamp = Date.now();
  const saveResp = await callAPI(`/api/categories/${CATEGORY_ID}/backgrounds`, 'POST', {
    name: `pipeline-audit-mar18-2-25pm-${timestamp}`,
    imageData: bg.imageData,
    mimeType: bg.mimeType,
    format: bg.format,
    width: bg.width,
    height: bg.height,
    promptUsed: 'Professional product photography scene - PIPELINE AUDIT March 18 2026 at 2:25 PM',
  });

  if (saveResp.status !== 201) {
    console.error('✗ Save failed:', saveResp.status, saveResp.raw || saveResp.data);
    process.exit(1);
  }

  console.log('✓ Background saved');
  const saved = saveResp.data.background;
  console.log(`  ID: ${saved.id}`);
  console.log(`  Name: ${saved.name}`);
  console.log(`  GDrive File ID: ${saved.gdrive_file_id}`);
  console.log(`  Storage path: ${saved.storage_path}`);
  console.log(`  Storage URL: ${saved.storage_url}`);

  // Step 2c: Inspect Supabase
  console.log('\n=== Step 2c: Inspect Supabase Metadata ===');
  const { data: dbRec } = await supabase.from('backgrounds').select('*').eq('id', saved.id).single();
  console.log('✓ Database record verified:');
  console.log(`  ID: ${dbRec.id}`);
  console.log(`  category_id: ${dbRec.category_id}`);
  console.log(`  user_id: ${dbRec.user_id}`);
  console.log(`  company_id: ${dbRec.company_id}`);
  console.log(`  name: ${dbRec.name}`);
  console.log(`  slug: ${dbRec.slug}`);
  console.log(`  format: ${dbRec.format}`);
  console.log(`  width x height: ${dbRec.width}x${dbRec.height}`);
  console.log(`  storage_provider: ${dbRec.storage_provider}`);
  console.log(`  storage_path: ${dbRec.storage_path}`);
  console.log(`  storage_url: ${dbRec.storage_url}`);
  console.log(`  gdrive_file_id: ${dbRec.gdrive_file_id}`);
  console.log(`  created_at: ${dbRec.created_at}`);
  console.log(`  Has company prefix: ${dbRec.storage_path.startsWith('Sunday Natural/') ? '✅ YES' : '❌ NO'}`);

  // Step 2d: Verify Google Drive
  console.log('\n=== Step 2d: Verify Google Drive File ===');
  try {
    const { data: fileInfo } = await drive.files.get({
      fileId: saved.gdrive_file_id,
      fields: 'id, name, size, createdTime, mimeType, parents',
      supportsAllDrives: true,
    });
    console.log('✓ File exists in AdForge Storage Shared Drive');
    console.log(`  File ID: ${fileInfo.id}`);
    console.log(`  Name: ${fileInfo.name}`);
    console.log(`  Size: ${(parseInt(fileInfo.size) / 1024).toFixed(1)} KB`);
    console.log(`  Created: ${fileInfo.createdTime}`);
    console.log(`  MIME: ${fileInfo.mimeType}`);
    
    // Verify it's in the right folder
    console.log(`  Parent folders: ${fileInfo.parents ? fileInfo.parents.join(', ') : 'none'}`);
  } catch (err) {
    console.error('✗ File not found in Drive:', err.message);
    process.exit(1);
  }

  // Save for next steps
  fs.writeFileSync('/tmp/pipeline-audit-data.json', JSON.stringify({
    categoryId: CATEGORY_ID,
    backgroundId: saved.id,
    backgroundGdriveId: saved.gdrive_file_id,
    backgroundName: saved.name,
    backgroundPath: saved.storage_path,
    timestamp: new Date().toISOString(),
  }, null, 2));

  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('✓ STEP 2 COMPLETE - NEW Background Generated & Saved');
  console.log(`\nAsset created: ${saved.name}`);
  console.log(`Created at: ${dbRec.created_at}`);
  console.log(`View in Drive: https://drive.google.com/file/d/${saved.gdrive_file_id}/view`);
  console.log(`Direct link: ${saved.storage_url}`);
  console.log(`Storage path: ${saved.storage_path}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
}

main().catch(err => {
  console.error('\n✗ ERROR:', err.message);
  console.error(err.stack);
  process.exit(1);
});
